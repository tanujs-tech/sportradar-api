...
module Sportradar
  module Api
    class Images < Request
      ...
      def player_manifests(year = Date.today.year)
        response = if league
                     if year != Date.today.year
                       ...
                       get request_url("#{league}/#{image_type}/players/manifest")
                     end
                   elsif nfl_premium || usat_premium
                     get request_url("#{image_type}/players/#{year}/manifest")
                   else
      ...
      alias all_players player_manifests
      # Coach Manifests
      ...
      alias all_coaches coach_manifests

      def venue_manifests
        raise Sportradar::Api::Error::InvalidLeague unless league.nil?
        response = if version == 3
                     get request_url('venues/manifest')
                   else
      ...
      alias all_venues venue_manifests
      ...
      alias all_events event_manifests
      alias live_images event_manifests
      ...
      alias player_images image_url
      alias coach_images image_url
      alias venue_images image_url
      ...
      def v2_api_sports
        %w[golf nascar nba nfl nhl ncaafb ncaamb mls]
      end

      def v3_api_sports
        %w[mlb soccer cricket f1 rugby tennis]
      end
